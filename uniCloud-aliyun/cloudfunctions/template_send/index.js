...
exports.main = async (event, context) => {
	...
	var coverIds = event.coverIds
	if(coverIds){
		for (var i = 0; i < coverIds.length; i++) {
			var sendCoverId = coverIds[i]
			const coverList = await db.collection('wx_cover').doc(sendCoverId).get()
			...
			var pages = cover.status==1?'/pages/index/index?index=1&detailId='+cover._id:'/pagesOther/system/write'
			var param = {
				thing1: {
					value: name
				},

		...
		<view class="">
			<view class="u-flex u-row-left u-flex-wrap u-p-l-40 u-p-t-20">
				<view class="tab u-skeleton-fillet" @longpress="moreClick(item._id, true)" @click="detail(item._id, item.status)" v-for="(item, index) in coverList" :key="index">
					<image :src="item.status!=1?'/static/shenhe.jpg':vuex_compressPicture?item.image+'?x-oss-process=image/resize,w_200/quality,q_60':item.image" style="width: 215rpx;height: 350rpx;" mode="aspectFill"></image>
